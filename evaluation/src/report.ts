...
export function renderReport(input: ReportInput): string {
  const { goldset, coverage, grounding, evidence, candidates, relationships, cost } = input;
  const lines: string[] = [];
  ...
  lines.push('');
  lines.push(
    `Generated ${input.generatedAt.toISOString()} against collection **${input.collectionName}** (\`${input.collectionId}\`), scored on \`evaluation/goldset.json\` version ${goldset.version}.`,
  );
  lines.push('');
  lines.push(
    `The gold set holds **${goldset.claims.length} claims and ${goldset.pairs.length} pairs** across ${goldset.documents.length} documents. It is development data: it was read before the system was built. The held-out measurement is the generalization run in section 8.3.`,
  );
  lines.push('');
